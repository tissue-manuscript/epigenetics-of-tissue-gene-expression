...
metadata <- read.table("ExGenes_metadata.csv", sep = ",", row.names = 1, header = T)
read_Count <- read.table("Exercise_regulated_genes_FPKM.csv", header = T, sep = ",", row.names = 1)
...
# Create matrix
dds <- DESeqDataSetFromMatrix(countData=round(read_Count),
                                      colData = metadata,
                                      design = ~ 1)
